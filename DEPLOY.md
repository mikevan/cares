...
| `ENABLE_TRANSLATION` | No | `false` | Off by default. Turning this on sends the rendered HTML of every page -- including transaction memos and member/donor PII -- to a third-party API (Groq) for translation whenever a visitor's browser requests a non-English language. Currency amounts and GL account numbers are masked before the call and restored afterwards, so no figure is ever at the model's mercy (`services/translation_masking.py`); names, addresses and memos are not masked and do leave the building. Requires `GROQ_API_KEY`. Only enable this after reading `services/translation_service.py` and deciding that's acceptable for your chapter's data. |
| `GROQ_API_KEY` | Only when `ENABLE_TRANSLATION=true` | none | Groq API key. Without it translation is skipped silently and every page serves in English. The API bill for this key is the chapter's. |
| `GROQ_TRANSLATION_MODEL` | No | `openai/gpt-oss-20b` | Override the translation model. Verify a replacement with `python scripts/check_translation_masking.py` before a demo -- a retired model fails silently, and a model that mangles the masking placeholders makes every page fall back to English. |
| `TRANSLATION_MODE` | No | `segments` | `segments` sends the whole page for context but asks the model to return only the translated strings, numbered; the page is reassembled locally by character offset, so markup, CSS and JavaScript are never regenerated. `page` reverts to asking for the whole document back. Segment mode translates roughly three times larger pages, costs less, and removes the possibility of the model damaging the markup. |
| `GROQ_REVIEW_MODEL` | No | `openai/gpt-oss-120b` | Model for the accounting review pass. Groq's rate limits are per organization **and per model**, so running the two passes on different models draws on two buckets instead of exhausting one — which is what a full warm run does otherwise. The larger model is also the better reviewer. Set it equal to `GROQ_TRANSLATION_MODEL` to trade that for prompt-prefix caching across the two passes instead. |
| `ENABLE_TRANSLATION_REVIEW` | No | `true` | Second pass over each translation, checking it against an accounting-terminology rubric before it is cached. Doubles the API cost and the latency of a cache *miss*; costs nothing on a hit, because the reviewed page is what gets stored. Set to `false` to translate in one pass. |
