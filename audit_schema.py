...
def ensure_audit_log_organization_column(connection):
    """audit_log.organization_id must exist before the trigger writes it.

    Also created by rls_schema.install_rls(); duplicated here so installing
    the audit triggers alone cannot leave the trigger writing to a column
    that is not there."""
    connection.execute(text(
        "ALTER TABLE audit_log ADD COLUMN IF NOT EXISTS organization_id INTEGER"))
    connection.execute(text(
        "CREATE INDEX IF NOT EXISTS idx_audit_log_org_id ON audit_log(organization_id, id)"))
...
def install_audit_triggers(connection):
    ...
    ensure_audit_log_organization_column(connection)
